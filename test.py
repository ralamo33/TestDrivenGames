...
class DoubleJumpTests(unittest.TestCase):
    def setUp(self):
        self.board = Game()
        self.board.move(Move(2, 1, 3, 0))
        self.board.move(Move(5, 0, 4, 1))
        self.board.move(Move(2, 3, 3, 2))
        self.board.move(Move(5, 2, 4, 3))
        self.board.move(Move(1, 2, 2, 3))
        self.board.move(Move(4, 3, 2, 1))

    def test_double_jump(self):
        double_jump_piece = self.board.get_space(1, 0).piece
        self.board.move(Move(1, 0, 3, 2))
        self.board.move(Move(3, 2, 5, 0))
